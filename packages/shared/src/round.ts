 ...
 * round. Offline members don't block the flip (they reveal as ?), but
 * an empty table or a round with nobody online never counts as ready.
 */
export function everyoneVoted(session: Session): boolean {
  ...
  const online = Object.entries(session.users ?? {}).filter(([, u]) => u?.name && u.online);
